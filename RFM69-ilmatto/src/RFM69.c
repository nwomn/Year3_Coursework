...
uint8_t own_canSend_p0()
{
    if (mode == RF69_MODE_RX && PAYLOADLEN == 0)
    {
        while (readRSSI(0) > CSMA_LIMIT)
        { // Channel not idle, then wait
            millis_current = millis();
            unsigned long random_time = rand() % 100;
            // srand(time(NULL));
            printf("%d\n", readRSSI(0));
            // printf("%d\n", random_time);
            while (millis() - millis_current < random_time)
                ; // wait random time
        }
        setMode(RF69_MODE_STANDBY);
        return 1;
...
uint8_t own_canSend_pp(uint16_t p)
{
    if (mode == RF69_MODE_RX && PAYLOADLEN == 0)
    {
        while (readRSSI(0) < CSMA_LIMIT)
        {
            ...
            while (millis() - millis_current < 1000)
                ; // wait 1000 ms
        }
        while (millis() - millis_current < 1000)
            ; // wait 1000 ms
...
void send(uint8_t toAddress, const void *buffer, uint8_t bufferSize)
{
    ...
    while (!own_canSend_p0() && millis() - millis_current < RF69_CSMA_LIMIT_MS)
        receiveDone();
